...
class WCNFFormula(object):
    ...
    def transform_1_3_clausule(self,clausule,type,weight1_3):            
        if len(clausule) > 3:
            if type == "soft":
                b = self.new_var()
                self.add_clause([-b],weight=weight1_3)
                clausule.append(b)
            new_literal = []
            while len(clausule) >= 3:
            ...
            if type == "hard":
                while len(clausule) < 3:
                    ...
                    self.new_var()
                self.add_clause(clausule,weight=TOP_WEIGHT)
            else:
                if len(clausule) ==  1:
                    self.add_clause(clausule,weight=weight1_3)
                else:
                    b = self.new_var()
                    self.add_clause([-b],weight1_3)
                    clausule.append(b)
                    self.add_clause(clausule,weight=TOP_WEIGHT)
